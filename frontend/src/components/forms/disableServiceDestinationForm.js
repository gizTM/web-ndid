  ...
  return {
    onSubmitClick: (event, value, loading) => {
      if (loading) {
        ...
        post("/api/disableServiceDestination", {
          node_id: value.node_id,
          service_id: value.service_id
        })
          .then(data => {
            NotificationManager.success(
              "disabling service destination succeeded",
              "Form submitted!"
            );
          ...
          .catch(err => {
            NotificationManager.error(
              ...
              "Error disabling service destination",
              5000
            );
            dispatch(callLoading(false));
          });
...
class DisableServiceDestinationForm extends React.Component {
  render() {
    const labelWidth = 5;
    ...
    return (
      ...
      >
        <Label>{MENU.DISABLE_SERVICE_DESTINATION}</Label>
        <hr />
